// components/AboutManagement.tsx
'use client';
      ...
      if (response.ok) {
        ...
        if (data.success) {
          const formattedData: AboutContent = {
            ...
            teamMembers: Array.isArray(data.data.teamMembers) 
              ? data.data.teamMembers.map((m: any) => ({
                  id: m.id,
                  name: m.name || 'Unknown',
                  role: 'Local Expert',
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (response.status === 401) {
        ...
        throw new Error(`Failed to save about content: ${response.status}`);
      }
    }
  } catch (error) {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className={`text-sm mt-2 ${textSecondary}`}>
            <strong>Note:</strong> "Our Providers" section automatically shows active providers from the system
          </p>
        </div>
          ...
          {editing ? (
            <div className="space-y-8">
              ...
              <div className="pt-8 border-t border-gray-700">
                <div className="mb-6">
                  <h3 className="text-xl font-bold mb-4">Our Providers (Team Members)</h3>
                  <p className={`text-sm ${textSecondary} mb-4`}>
                    Select providers to feature in the "Our Providers" section. These will be shown as team members on the About Us page.
                  </p>
