...
const getDefaultAboutData = (): AboutData => ({
  ...
  teamMembers: [
    {
      id: '1',
      name: 'Local Expert Team',
      role: 'Experience Provider',
      bio: 'Our team of local experts ensures authentic and memorable travel experiences.',
      imageUrl: 'https://ui-avatars.com/api/?name=Local+Expert&background=2196F3&color=fff',
      email: 'experts@tourismhub.com',
      activityCount: 10
    }
  ],
...
const AboutPage: React.FC = () => {
        ...
        if (result.success && result.data) {
          ...
          const teamMembers = Array.isArray(result.data.teamMembers) 
            ? result.data.teamMembers.map((member: any) => ({
                id: member.id || `member-${Math.random()}`,
                name: member.name || 'Local Expert',
                role: member.role || 'Experience Provider',
          ...
          const values = Array.isArray(result.data.values) 
            ? result.data.values 
            : getDefaultAboutData().values;
          
          const aboutWithTeamMembers: AboutData = {
            ...
            description: result.data.description || 'Connecting travelers with authentic experiences',
            mission: result.data.mission || 'To make travel accessible to everyone',
            vision: result.data.vision || 'To be the most trusted travel platform',
            values: values,
            teamMembers: teamMembers.length > 0 ? teamMembers : getDefaultAboutData().teamMembers,
  ...
  const fetchStats = async () => {
    try {
      ...
      const [activitiesRes, allUsersRes, bookingsRes] = await Promise.all([
      ...
      if (allUsersRes.ok) {
        ...
        if (Array.isArray(allUsers)) {
          ...
          totalProviders = allUsers.filter((user: any) => 
            user.role === 'Provider' || user.role === '1'
          ).length;
          
          totalTravelers = allUsers.filter((user: any) => 
            user.role === 'Tourist' || user.role === '2' || 
            (!user.role && user.email && !user.email.includes('_deleted_'))
          ).length;
        }
      }
      ...
      const estimatedHappyCustomers = Math.max(
        totalBookings * 3, 
        totalTravelers * 0.7 
      );
      ...
      setStats({
        ...
        totalProviders,
        totalTravelers,
        happyCustomers: Math.floor(estimatedHappyCustomers)
      });
      
    } catch (error) {
      console.error('Error fetching stats:', error);
      // Përdor statistikat default
      setStats({
        totalActivities: 50,
        totalProviders: 20,
        totalTravelers: 300,
      ...
      <div className="max-w-6xl mx-auto px-4 -mt-8 relative z-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            ...
            <div key={index} className="bg-white rounded-2xl p-6 shadow-xl text-center hover:shadow-2xl transition-shadow">
        ...
        <div className="max-w-6xl mx-auto px-4 py-16">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {aboutData.teamMembers.map((member, index) => (
              <div key={member.id || `member-${index}`} className="group">
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="text-center mt-12 pt-8 border-t border-white/20">
            ...
            <p className="text-blue-100 text-sm mt-2">
              © {new Date().getFullYear()} TourismHub. All rights reserved.
            </p>
          </div>
        </div>
      </div>
