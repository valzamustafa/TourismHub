...
const fetchActivitiesWithImages = async () => {
  try {
      ...
      activitiesData.map(async (activity: any) => {
        try {
          ...
          if (imagesResponse.ok) {
            ...
            images = imagesData.data?.map((img: any) => img.imageUrl) || [];
          }
          
          return {
            ...activity,
            images: images.length > 0 ? images : ['/images/default-activity.jpg']
          };
        } catch (error) {
    ...
    console.error('Error fetching activities with images:', error);
...
useEffect(() => {
  ...
  }, [categories, activities, selectedCategory, filteredActivities, showActivities]);
